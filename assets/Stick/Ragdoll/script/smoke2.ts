...
@ccclass
export default class smoke2 extends cc.Component {
    ...
    die() {
        cc.log("which bound: ",Global.player2_dead_bound);
        if (Global.player2_dead_bound == 1) {           // up bound
            this.node.angle = 90;
        } else if (Global.player2_dead_bound == 3) {    // floor
            this.node.angle = -45;
            this.node.setPosition(this.node.x + 200, this.node.y);
        } else if (Global.player2_dead_bound == 4) {    // left bound
            this.node.angle = -90;
        ...
        this.smokeAni.play("smoke_die_2");
